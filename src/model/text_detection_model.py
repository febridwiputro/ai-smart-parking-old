...
class TextDetector:
    ...
    def image_restoration(self, image):
        # Check if the image is empty
        if image is None or image.size == 0:
            print("[Image Restoration] The input image is empty.")
            return np.array([])  # Return an empty array or handle it as needed

        # Check the number of channels in the input image
        if len(image.shape) < 3 or image.shape[2] != 3:
            print("[Image Restoration] Invalid input image format. Expected BGR image with 3 channels.")
            return np.array([])  # Return an empty array or handle it as needed
        ...
        # Apply the GAN model for super resolution
    ...
    def text_detect_and_recognize(self, image, bg_color):
        """
        ...
        if len(cropped_images) > 0:
            ...
            final_plate = self.cr.process_image(cropped_images, bg_color)
    ...
    def recognition_image_text(self, image, bg_color):
