...
from src.model.plat_model import PlateDetector
...
class OCRController:
    def __init__(self, ard, matrix_total, vehicle_detection_model, character_recognition, plate_detection_model):
        ...
        self.plat_detector = PlateDetector(plate_detection_model)
    ...
    def get_process_plat_image(self, car) -> (np.ndarray, np.ndarray):
        if car.shape[0] == 0 or car.shape[1] == 0:
            return np.array([])
        ...
        if plat.shape[0] == 0 or plat.shape[1] == 0:
            return np.array([]), np.array([])
        plat_preprocessing = self.td.image_restoration(plat)
        return plat_preprocessing, plat
    ...
    def image_to_text(self, frame):
        ...
        # Check if the frame has the correct number of channels
        if len(frame.shape) < 3 or frame.shape[2] != 3:
            # If it's already a grayscale image (1 channel), you can use it directly
            img_gray = frame if len(frame.shape) == 2 else cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        else:
            img_gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)

        bg_color = check_background(img_gray)

        td_method = "easyocr_readtext"  # Adjust this as needed
        if td_method == "easyocr_readtext":
            text, results, box = self.ocr.recognition_image_text(frame, bg_color)
            if not text:
                return "", box
            ...
            text = self.ocr.text_detect_and_recognize(frame, bg_color)
            if not text:
                return "", []
